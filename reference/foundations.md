...
The visual system. Lock these once per project (step 4 of the procedure) and never let them drift between sections. Everything here is a default to apply, not a menu to deliberate over.
...
|---|---|
| **Display** (sans) | Geist, Satoshi, Cabinet Grotesk, Clash Display, PP Neue Montreal, General Sans, GT Walsheim, Söhne Breit |
...
| **Body** | Geist, Inter Tight, Satoshi, General Sans, Switzer, system-ui (for true minimal) |
| **Mono** | Geist Mono, JetBrains Mono, IBM Plex Mono, Söhne Mono |

- **Inter is discouraged as a *default*** (it's the AI tell) — acceptable when the brief explicitly wants neutral/Linear-style or is accessibility-first.
...
```css
--space-1: 0.25rem;  /* 4  */   --space-5: 1.5rem;   /* 24 */
--space-2: 0.5rem;   /* 8  */   --space-6: 2rem;     /* 32 */
--space-3: 0.75rem;  /* 12 */   --space-8: 3rem;     /* 48 */
--space-4: 1rem;     /* 16 */   --space-12: 6rem;    /* 96 — section */
```
...
- **One accent**, saturation effectively <80% (`C ≲ 0.16` in OKLCH). One gray family — never mix warm and cool grays in one project. Lock the accent across the whole page (no surprise blue CTA on a warm-grey site).
...
Every section has **one** focal point. Establish it with, in order: **size → weight → color/contrast → space → motion.** Don't fight a hierarchy with four competing emphases.
