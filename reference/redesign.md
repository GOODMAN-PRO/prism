...
2. **Diagnose.** Run the audit below. List every generic pattern, weak point, and missing state — name files and lines.
3. **Fix.** Apply upgrades in the priority order, testing after each. Preserve identity where committed brand colors/fonts exist (identity-preservation wins over taste).
...
3. **Hover/active/focus states** — makes it feel alive: bg shift + `scale(0.97)` active + visible focus ring + 200–300ms transitions.
...
**Color & surface:** pure `#000`/`#fff` → tinted off-black/off-white; >1 accent → pick one; warm+cool grays mixed → one family; AI purple gradient → neutral base + one considered accent; black box-shadows → tinted to bg hue; flat/sterile → subtle noise or ambient gradient; a lone dark section in a light page → commit to one theme.
...
**Interactivity:** no hover/active → add; instant transitions → 200–300ms; missing focus ring → add (a11y, not optional); spinner-only loading → skeletons; dead `#` links → real or disabled; no active-nav indicator → style it; layout-property animation → `transform`/`opacity`.
...
Work with the existing stack. Don't break functionality — test after every change. Check the dependency file before importing. Check Tailwind version before touching config. Keep changes focused and reviewable. When done, run `preflight.md`.
